...
//   벽지 종류는 이제 이 카드 위에 있는 벽지 카드(PaperPicker)에서 고른다. 여기는 평형·베이만
//   담당한다(범위·지역·상태 칩은 ConditionChips가 PaperPicker 아래에 붙는다).
//
// 2026-09-15 형아 지시(㎡ 모드 추가): 평형 칩만 있던 자리를 공용 부품 AreaInput(평/㎡ 토글 +
//   칩 + 직접 입력 + 환산 캡션)으로 바꿨다.
//
// 2026-09-15 디자인 통일 지시(계산기 3종 화면 정리):
//   이 카드가 직접 보여주던 즉답 큰 숫자(금액·롤수·근거줄)를 통째로 없앴다 — 바로 옆(PC)·
//   아래(모바일)에 있는 ResultPanel이 이미 똑같은 숫자를 보여주고 있어서 한 화면에 같은
//   금액이 두 번 보이는 중복이었다(검사관 지적 — "246만~308만원 · 46롤"이 두 곳에 있었다).
//   결과는 ResultPanel 한 곳에서만 보여준다("결과 카드 하나만" 원칙). 이 카드는 입력만
//   담당하고, 입력값 자체가 범위 밖일 때(5평 미만 등)만 짧게 알려준다(중복 아닌 고유 정보).
//   테두리 카드(Card)도 없앴다 — 카드 속 카드 금지, 구획 제목+여백으로만 나눈다.
//
// 작성일: 2026년 09월 08일
