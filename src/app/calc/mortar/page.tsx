...
const FAQS: CalcFaqItem[] = [
  ...
  },
  {
    ...
    a: '배관을 완전히 덮는 최소 두께를 고려해 보통 40~50mm를 씁니다. 단열재 위에 시공할 때는 균열·침하를 막기 위해 50mm 이상을 권장하는 경우가 많아요. 계산기 "방통" 칩을 누르면 45mm가 기본값으로 채워져요.',
  },
  {
    q: '셀프레벨링은 몇 포나 필요한가요?',
  ...
  {
    q: '미장 인건비는 어떻게 계산하나요?',
    a: '오늘 하루 몇 명이 붙는지로 계산해요 — 손미장·장비 타설 둘 다 물량을 하루 안에 끝낸다는 전제로 필요 인원을 구하고, 여기에 미장공·보통인부 노임단가(시장가~표준품셈 범위)를 곱해요. 장비 타설을 고르면 장비대(50만~70만원/일)와 정벌 마무리(피니싱) 인건도 같이 더해요. 셀프레벨링은 인건비 분리 근거가 없어 자재비만 계산하고 시공비는 현장 견적으로 안내해요.',
  },
  {
    q: '손미장과 장비 타설, 뭐가 다른가요?',
    a: '손미장은 미장공·보통인부가 흙손으로 바르는 방식이라 아파트·빌라처럼 레미콘차가 들어오기 어려운 국소 현장에 맞아요(방통도 대부분 이 방식이에요). 장비 타설은 모르타르 펌프로 넓게 뿌리는 방식이라 레미콘차 진입이 가능한 상업·건축 현장에 유리하고, 대신 장비 임대료(50만~70만원/일)가 따로 붙어요. 계산기 기본값은 손미장이고, "공법" 칩으로 직접 바꿀 수 있어요.',
  },
  {
    q: '20평 방통(두께 100mm) 손미장 인건은 몇 명인가요?',
    a: '20평(66㎡)×100mm 기준으로 기공 2명·조공 2명이 하루 안에 끝내요. 면적이나 두께가 늘면 이 기준 물량(약 6.6㎥) 대비 배수로 인원이 늘어나는 식이라, 40평이면 대략 기공 4명·조공 4명이 필요해요 — 날짜를 늘리는 대신 인원을 늘려서 하루에 끝내는 게 원칙이에요.',
  },
];
