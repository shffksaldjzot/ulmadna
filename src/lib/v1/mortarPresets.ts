...
/**
 * 용도별 두께 범위 + 기본 공법 (06_미장.md §1 표 · §11-9 공법 선택 원칙).
 * 표준품셈에는 "바닥 미장 두께" 자체를 규정한 항목이 없어(9-1-2는 벽체·24mm 이하 기준)
 * 방통(B등급, 삼표 공식 블로그 근거)을 빼면 전부 인테리어 실무 관행값(C등급)이다.
 *
 * 2026-09-15 운영자 현장 기준 지시(§11-9): 기본 공법은 **용도와 무관하게 항상 손미장**이다 —
 * 방통(난방배관 매립)도 아파트·국소 현장이 대부분이라 레미콘차가 들어오기 어렵고,
 * 장비 타설은 상업·건축 현장처럼 레미콘차 진입이 가능할 때만 쓰는 예외다. 예전엔
 * "방통 전체"만 기본이 장비 타설이었는데(2026-09-14 검사관 결정), 운영자 현장 기준의 최신 현장
 * 기준으로 뒤집혔다 — 이 필드는 이제 4개 용도 전부 '손미장'이지만, 용도별로 다시
 * 갈릴 가능성에 대비해 필드 자체는 남겨 둔다.
 */
export const USAGE_PRESET: Record<MortarUsage, UsagePreset> = {
  방통전체: { label: '방통 전체', minMm: 40, maxMm: 50, defaultMm: 45, defaultMethod: '손미장', grade: 'B' },
...
/** 공법 칩 옆에 붙이는 캡션 1줄(간단·정밀 모드 공용) — 06_미장.md §11-9 */
export const METHOD_CAPTION = '아파트·국소 시공은 손미장, 레미콘차 진입되는 상업·건축은 장비 타설';
 ...
 * 2026-09-15 운영자 현장 기준 피드백: 방통은 현장에서 50~150mm까지도 흔히 쓴다 — 06_미장.md 표(10~50)
...
/**
 * 레미탈 두께 입력 상한(mm). 2026-09-15 운영자 현장 기준 피드백(현장 경험): 방통은 50~150mm까지 흔하다 —
...
/**
 * 장비 타설일 때 화면에 보여줄 안내.
 * 2026-09-15 운영자 현장 기준 지시(06_미장.md §11-1)로 장비대(50만~70만원/일)를 이제 "5층 비용 구성"에
 * 실제로 계상한다 — 예전엔 "장비비 별도"로만 안내했는데, 그러면 총액에 안 잡혀서 견적이
 * 실제보다 작게 보이는 문제가 있었다. 이제는 비용에 포함됐다는 걸 알리는 문구로 바꿨다.
 * 즉답(useMortarQuickCalc)·서버(mortar.ts) 둘 다 이 문구 하나를 그대로 쓴다.
 */
export const EQUIPMENT_RENTAL_NOTE = '장비대는 현장 접근성·거리에 따라 달라질 수 있어요';

/** 배송비·양중비·지게차 하차비 직접입력 상한(원) — 비정상 값(문자열·음수·억 단위 오타) 방어용 클램프 */
export const MONEY_INPUT_WON_MAX = 10_000_000;

/**
 * 지게차 하차비 기본값(원) — 2026-09-15 운영자 현장 기준 확정: 조사값(2만~3만원, C등급)을 버리고
 * 운영자 현장 기준 10만원/회(B등급)로 교체. 정확 모드에서 토글을 켜면 이 값이 채워지고,
 * 사용자가 금액을 직접 수정할 수 있다(06_미장.md §11-2도 이 값으로 고쳤다).
 */
export const FORKLIFT_DEFAULT_FEE_WON = 100000;
