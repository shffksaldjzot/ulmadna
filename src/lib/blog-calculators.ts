...
// - 여러 계산기 키워드가 같은 글에 걸리면 CALCULATORS에 적힌 순서대로 먼저 걸리는 키를
//   돌려준다(detectCalculator의 for문 순서 = Record 선언 순서). 도배 → 바닥재 순으로 둔다.
//
...
export const CALCULATORS: Record<CalculatorKey, CalculatorInfo> = {
  ...
  flooring: {
    ...
    keywords: ["바닥재", "마루", "강마루", "강화마루", "장판", "데코타일"],
  },
};
