...
/**
 * 도배풀 14kg 포 하나가 감당하는 **도배 면적** (평 = 벽+천장 ㎡ ÷ 3.3058).
 *
 * 설계 정본의 "합지 20~30평 / 실크 약 20평"은 공급 평형 기준이었다(2026-09-09 확인).
 * 엔진은 도배 면적 평(workPyeong)을 넣으므로 공급→도배면적 환산(34평 → 76.1도배평, ×2.24)을
 * 미리 곱해 둔다. 안 그러면 34평 실크가 4포로 나와 실제(2포)의 두 배가 된다.
 *
 * 검증(2026-09-09): 실크 2포 × 1:1 희석 = 완성풀 56kg ÷ 313㎡ = 0.18kg/㎡.
 * 가루풀 기준(고려풀 200g + 물 3L = 실크 1롤 16.5㎡ → 0.19kg/㎡)과 일치.
 */
export const PASTE_PYEONG_PER_BAG: Record<PaperType, CoefficientRange> = {
  // 합지는 풀을 묽게 타서(물 2 대 풀 1) 더 넓게 쓴다
  합지: { min: 45, max: 65, grade: 'A', source: '설계 정본 2-D절 (14kg 포 = 공급 20~30평, 물 1:2 희석) × 도배면적 환산 2.24' },
  // 실크는 되게 타서(1 대 1) 소요가 많다
  실크: { min: 45, max: 45, grade: 'A', source: '설계 정본 2-D절 (14kg 포 = 공급 약 20평, 물 1:1 희석) × 도배면적 환산 2.24. 가루풀 롤 기준(0.19kg/㎡)과 교차 검증' },
};
...
/** 부직포 1롤 면적 (㎡) — 110cm × 80m 기준 */
export const NONWOVEN_SQM_PER_ROLL: Coefficient = {
  value: 88,
  grade: 'A',
  // 2026-09-09 형아 엑셀 대조표(01-3_벽지엑셀_대조표_20260909.md) "바로 반영 추천" 반영.
  // 90m 롤은 실제 시장에 없고(엑셀 표본 3건 = 110×80m 88㎡ / 120×80m 96㎡ / 108×84m 90.72㎡),
  // 값이 크면 필요 롤수를 적게 잡아 견적이 낮게 나오는 문제가 있어 가장 대표인 88㎡로 낮춘다.
  source: '형아 엑셀 대한민국_벽지_부자재_DB_2026-09-09(온라인 유통가) — 110cm × 80m 88㎡(표본 3건 중 대표값)',
};

/**
 * 초배지 — 실크 벽지 1롤당 이음용 초배지 세트(5장) 1개.
 * 2026-09-09 형아 결정: 예전 "합지 각초배지 30cm×90m 롤 = 벽면적 × 0.3(추정)"은
 * 30cm 각초배지가 시장(형아 엑셀·웹)에 없어서 버리고, 유통 단위 그대로 실크 롤 기준으로 바꿨다.
 * 합지는 초배지 항목 없음.
 */
export const LINING_SET_PER_ROLL: Coefficient = {
  value: 1,
  grade: 'B',
  source: '형아 엑셀 대한민국_벽지_부자재_DB_2026-09-09 "실크벽지 1롤용 초배지 세트(5장)" — 롤수 × 1세트',
};
...
/**
 * 본드 1개 용량 (kg) — 목공본드 800g 기준.
 * 2026-09-09 형아 결정: 도배용 5kg 캔은 시장에 없고(5kg은 바닥 타일 본드) 도배 부자재는
 * 200g·800g 소포장으로 유통되므로 800g 단위로 바꿨다.
 */
export const BOND_KG_PER_CAN: Coefficient = {
  value: 0.8,
  grade: 'A',
  source: '형아 엑셀 대한민국_벽지_부자재_DB_2026-09-09 (오공 목공본드 705 800g)',
};
...
export const LABOR_WON_PER_PYEONG: Record<PaperType | '실크고급', Coefficient> = {
  합지: {
    value: 17000,
    grade: 'B',
    source: '2026-09-09 형아 확정: 34평(도배면적 76.1평) 합지 인건 128만원 기준 → 17,000원/도배평 (세부설계서 리서치값 15,000에서 상향)',
  },
  실크: {
    value: 28000,
    grade: 'B',
